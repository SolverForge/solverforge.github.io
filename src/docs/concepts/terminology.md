...
### Value Range
The set of possible values for a planning variable. In the common stock runtime, the planning variable names its provider with `value_range_provider = "solution_field"`.
...
### Constraint Stream
The fluent API for defining constraints. Streams start from
`ConstraintFactory::<Solution, Score>::new()`, then usually
`for_each(...)` with a generated solution source such as `Schedule::shifts()`
or `Schedule::employees()`. Use `solverforge::stream::vec(...)` for lower-level
custom extractors.
...
### for_each
Start a constraint stream by iterating over items produced by a collection
extractor. Prefer generated solution sources for normal planning entity and
problem fact collections.

### Generated Collection Source
A method like `Schedule::shifts()` or `Schedule::employees()` generated by
`#[planning_solution]`. These sources preserve metadata used by localized
incremental scoring.
