...
`ConstraintFactory::<Solution, Score>::new()`, then usually a generated
collection accessor such as `.shifts()` or `.employees()`. Use `for_each(...)`
for lower-level custom extractors.
...
### for_each
Start a constraint stream by iterating over items produced by an extractor
closure. Prefer generated collection accessors for normal planning entity and
problem fact collections.

### Generated Collection Accessor
A method like `.shifts()` or `.employees()` generated by `#[planning_solution]`
for use on `ConstraintFactory`. These accessors preserve source metadata used
by localized incremental scoring.
