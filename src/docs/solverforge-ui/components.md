...
| `SF.createStatusBar(config)`  | `{el, bindHeader, updateScore, setLifecycleState, setSolving, updateMoves, updateConstraintDots, colorDotsByScore, colorDotsFromAnalysis}` | Score display with constraint indicators; pass `header` or call `bindHeader()` if it should toggle local solve/stop controls |
