...
- CLI version
- Scaffold runtime target (currently SolverForge crate target 0.8.12)
- Runtime source (crates.io: solverforge 0.8.12)
- UI source (crates.io: solverforge-ui 0.5.1)
...
Current scaffolds are intentionally thin. The generated frontend composes the
shipped `solverforge-ui` primitives instead of vendoring a template-specific
asset stack, and treats `SF.createSolver(...)` plus
`SF.rail.createTimeline(...)` as the canonical retained-lifecycle and
scheduling surfaces.
