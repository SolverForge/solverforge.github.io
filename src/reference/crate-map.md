...
| `solverforge-py`   | the tagged `v0.6.3` Python source line, compiled-model PyO3 binding, retained diagnostics, and embedded UI asset bridge | you want Python model authoring backed by the Rust solver engine |
...
- Start with the PyPI `solverforge` package when the application surface is
  ...
  `solverforge 0.6.3`; use the matching `solverforge-py v0.6.3` tag for source
  and example development on SolverForge 0.19.1.
...
| Python-authored planner            | PyPI `solverforge` package or the `solverforge-py v0.6.3` source tag |
