...
This post records the 0.5.0 release. Current projects should start from
`solverforge-cli`, which scaffolds one neutral application shell. Run
`solverforge --version` to see the exact scaffold targets in your installed
binary. Quickstarts and Python-binding timelines below are historical context,
not current onboarding guidance.
<% end %>
...
SolverForge is a constraint solver for planning and scheduling problems. It tackles complex optimization challenges like hospital scheduling, vehicle routing, resource allocation, and task assignment—problems where you need to satisfy hard constraints while optimizing for quality metrics.
...
<%= render Ui::Callout.new(title: "Note") do %>
...
<%= render Ui::Callout.new(title: "Beta Status", variant: "warning") do %>
...
- **Examples**: Quickstarts and examples are growing but not yet comprehensive
<% end %>
...
For a current concrete example, read the **[SolverForge Hospital Use Case](/docs/getting-started/solverforge-hospital-use-case/)**. It carries a full hospital scheduling application from the generic CLI scaffold through shifts, skills, availability constraints, retained jobs, and the browser UI.
...
This gives us the best of both worlds: predictable, high-performance solving in Rust, with accessible bindings for Python teams.

## What's Next

Beyond Python bindings, the tutorial roadmap includes:

- **SolverForge Hospital**: ✓ Available now
- **Vehicle Routing**: Next in pipeline
- More domain-specific examples as the project grows
...
Version 0.5.0 represents a turning point for SolverForge. The zero-erasure architecture and SERIO engine make high-performance, accessible constraint solving possible across languages while maintaining Rust's performance and safety guarantees.
...
The journey from FFI experiments to native Rust solver has been challenging, but the result is a constraint solver with concrete Rust internals and inspectable APIs.
