...
Current `solverforge-cli 2.0.4` scaffolds the `solverforge 0.11.1` runtime
target, so generated-app targets remain explicit.

For the current patch release, use
[SolverForge 0.11.1](/blog/releases/2026/05/05/solverforge-0-11-1/).
...
Use the current `solverforge-cli 2.0.4` scaffold target for new generated apps.
Move an older generated app to `solverforge 0.11.x` only when you are
deliberately upgrading that app's runtime target and validating the generated
code against the newer core crate.
